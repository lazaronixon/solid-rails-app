...
class User::Registration < Solid::Process
  deps do
    ...
    attribute :task_list_creation, default: Account::TaskLists::Creation
  end
  ...
  def create_user(email:, password:, password_confirmation:, **)
    ...
    return Continue(user: user) if user.persisted?

    input.merge!(user.errors) and Failure(:invalid_input, input:)
  end
