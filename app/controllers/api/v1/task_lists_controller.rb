...
module API::V1
  class TaskListsController < BaseController
    ...
    def index
      result = Account::TaskLists::Listing.call(account: current_account)

      data = result.value[:relation].pluck(*TASK_LIST_ATTRIBUTES).collect! { map_json_attributes(_1) }

      render_json_with_success(status: :ok, data:)
    end

    def create
      case Account::TaskLists::Creation.call(account: current_account, **task_list_params)
    ...
    def update
      case Account::TaskLists::Updating.call(account: current_account, id: params[:id], **task_list_params)
    ...
    def destroy
      case Account::TaskLists::Deletion.call(account: current_account, id: params[:id])
      in Solid::Failure(:task_list_not_found, _) then render_task_list_not_found
      in Solid::Failure(:inbox_cannot_be_edited, _) then render_cannot_update_or_delete_inbox
      in Solid::Success
